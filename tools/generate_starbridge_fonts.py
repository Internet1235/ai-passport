#!/usr/bin/env python3
"""Generate/check Starbridge subsets of LXGW WenKai Screen v1.522 (SIL OFL)."""
...
source = (ROOT / "main/starbridge_app.c").read_text()
symbols = "".join(sorted({ch for ch in source if ord(ch) > 127}))
for size in (12, 16):
    name = f"starbridge_{size}"
    ...
    if args.check:
        ...
        print(f"Starbridge {size}px: {len(symbols)} glyphs covered")
        continue
    ...
    target.write_text("/* Starbridge bitmap subset; source: LXGW WenKai Screen v1.522.\n"
