...
bool online_queue_extend(online_queue *q,online_packet *extra,unsigned count) {
    if(q->count || (count && !extra) || count>ONLINE_QUEUE_BASE)return false;
    q->extra=extra;q->extra_count=count;q->head=q->tail=0;return true;
}
